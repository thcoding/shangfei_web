...
for($j = 0; $j < $max;$j++){//依次取出每个叶子目录节点
				...
				while($arr = $mysql->fetch_array($res)){//得到courseunitversion_rel_attachment表中的一列
					//**获取courseunitversion id**
					$courseunitversionid = $arr["id"];
					//**获取lp_id**
					$lp_id = $arr["lpid"];
					...
					if($lesson_packageType!="SCORM/AICC"){//非"SCORM/AICC"课件，无学习过程
						...
						$returnInfo["list"]["courseUnit"][$i]["scormpath"] = $dirname."/".$href;
						$returnInfo["list"]["courseUnit"][$i]["userid"] = $user_id;
						$returnInfo["list"]["courseUnit"][$i]["courseunitversionid"] = $courseunitversionid;
						$returnInfo["list"]["courseUnit"][$i]["lpid"] = $lp_id;
					}
						...
						$href = $lesson_attachment["path"];
						
						$returnInfo["list"]["courseUnit"][$i]["type"] = 1;//scorm课件
						$returnInfo["list"]["courseUnit"][$i]["title"] = $lesson_courseunitName;
						$returnInfo["list"]["courseUnit"][$i]["url"] = $dirname.'/../admin/scorm/lp_view_mobile.php?id='.$lp_id.'&userid='.$user_id;
						$returnInfo["list"]["courseUnit"][$i]["status"] = $lesson_status;
						$returnInfo["list"]["courseUnit"][$i]["versionname"] = $lesson_courseunitversionName;
