...
$courseUnitVersionIds = $arr["courseunitids"];
...
<script type="text/javascript">
	   ...
	   $("#div_wait button").click(function(){
			if($(this).html()=="确定"){
				...
				$.ajax({				
					type: "POST",
					url: "fenpei.php",
					data:"type="+postType+"&courseUnit="+courseUnitIds+"&courseUnitVersion="+courseUnitVersionIds+"&courseUnitGroup="+courseUnitGroupIds+"&toCourseUnitGroup=<?php echo $_GET['id'];?>",
	...
	function setIframeSrc(type){
		postType = type;
		if(type==11){
			$("#pager_list").attr("src","courseUnitlist.php?aim=1");//为课程单元组分配课程单元			
		}else{
		...
		<div id="container">
			...
			<?php
				...
				while($arr = $mysql->fetch_array($res)){
					...
					if($courseunitinfo&&$courseunitinfo["deleted"]==0)
