		...
		<div id="container">
			...
			<?php
				...
				while($arr = $mysql->fetch_array($res)){
					if($arr["lpid"]){
						echo "<li><span><a href='javascript:;' onclick='if(confirm(\"确定要移除吗？\")){courseUnitIds = courseUnitIds.replace(\",\"+$arr[id]+\",\",\",\");f_remove(32,$arr[id],$_GET[id]);$(this).parent().parent().remove();}'><img src='../img/delete.png'></a></span><a href='scormShow.php?id=$arr[lpid]' target='_blank'><img src='../img/scorm.png' width='25' title='Scorm'>".$arr["title"]."</a></li>";
					}else{
						echo "<li><span><a href='javascript:;' onclick='if(confirm(\"确定要移除吗？\")){courseUnitIds = courseUnitIds.replace(\",\"+$arr[id]+\",\",\",\");f_remove(32,$arr[id],$_GET[id]);$(this).parent().parent().remove();}'><img src='../img/delete.png'></a></span><a href='courseUnitShow.php?id=$arr[id]'>".$arr["title"]."</a></li>";
					}
