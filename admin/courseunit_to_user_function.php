...
function getChildren($groupid,$grouptype){//获得group的children grouptype(1:用户（组）,2:课程单元（组），3：课程（组）)返回类型为item
	...
	if($grouptype==1){//获取用户
		...
		$arr = $mysql->fetch_array($res);
		$courseunitids = substr($arr["courseids"],1,-1);//获取所有课程单元id
		$res_courseunit = $mysql->query("select * from courseunit where deleted=0 and id in ($courseunitids) order by time desc");//根据courseid获取course表中的课程单元
