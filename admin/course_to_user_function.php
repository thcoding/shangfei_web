...
function getCourseorUserInfo(){//获取postinfo
	...
	else if(($returntype=="course_group")||($returntype=="user_group")||($returntype=="courseunit_group")){//返回的是组
		if($returntype=="course_group")
			$grouptype = 3;
		else if($returntype=="user_group")
			$grouptype = 1;
		else if($returntype=="courseunit_group")
			$grouptype = 2;
...
function getChildren($groupid,$grouptype){//获得group的children grouptype(1:用户（组）,2:课程单元（组），3：课程（组）)返回类型为item
	...
	if($grouptype==1){//获取用户
		...
		$arr = $mysql->fetch_array($res);
		$courseunitids = substr($arr["courseunitids"],1,-1);//获取所有课程单元id
		$res_courseunit = $mysql->query("select * from courseunitversion_rel_attachment where deleted=0 and id in ($courseunitids) order by time desc");//根据courseid获取course表中的课程单元
		while($arr_courseunit = $mysql->fetch_array($res_courseunit)){//逐行取出课程

			$userinfo = getUserinfoById($arr_courseunit["userid"]);
			$courseunitinfo = getCourseUnitinfoById($arr_courseunit["courseunitid"]);
			if($courseunitinfo["deleted"]==1)
				continue;
			//开始获取课程单元所需信息，并添加到item中
			$childrenitem["id"] = $arr_courseunit["id"];
			$childrenitem["title"] = $courseunitinfo["title"]."(".$arr_courseunit["versionname"].")";
			$childrenitem["time"] = $arr_courseunit["time"];
