...
//$oLP = new learnpath($lp_id,$userid);
...
function LMSGetValue(param)
{
	...
	// ---- cmi.core.student_id
		result='<?php echo $_SESSION["userid"]; ?>';
...
function LMSSetValue(param, val) {
	...
	if( param == "cmi.core.score.raw" ) {
		...
		return_value='true';
	} else if ( param == "cmi.session_time" ) {
		olms.session_time = val;
		olms.updatable_vars_list['cmi.core.session_time']=true;
		return_value='true';
...
*/
 ...
 // The cidReq is now very need
	var mysrc = 'lp_controller.php?action=content&lp_id='+olms.lms_lp_id+'&item_id='+next_item+'&'+'cidReq='+'';
