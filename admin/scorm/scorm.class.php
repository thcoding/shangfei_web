...
class scorm extends learnpath {
     ...
     */
     ...
     function import_manifest($course_code){
		...
		$new_lp = "lp";
		$new_lp_item = "lp_item";
        //logger("i've been here.2. organizations:" . json_encode($this->organizations));
		foreach($this->organizations as $id => $dummy)
		{
			...
			$sql = "INSERT INTO $new_lp (lp_type, name, ref, description, path, force_commit, default_view_mod, default_encoding, js_lib,display_order,session_id)" .
					"VALUES (2,'".$mysql->escape_string($myname).
                    "', '".$mysql->escape_string($oOrganization->get_ref()).
                    "','','".$mysql->escape_string($this->subdir).
                    "', 0, 'embedded', '".$this->manifest_encoding.
                    "','scorm_api.php',$dsp,$session_id)";
			...
			//$sql = "insert into courseunit (title,time,lpid,userid) values ('".$mysql->escape_string($myname)."','".date("Y-m-d H:i:s")."','".$lp_id."','$userid')";
			//die($sql);
			//$mysql->query($sql);

			//die("insert into courseunitversion_rel_attachment (courseunitid,attachmentid,lpid,versionname,userid,time) values ('$courseunitId','$attachmentId','$lp_id','".$versionname."','$userid','$time')");
            //logger("i've been here.1");
			...
			//$sql = "insert into courseunitversion (courseunitid,versionname,time,userid) values ('".$mysql->insert_id()."','��һ���汾','".date("Y-m-d H:i:s")."','".$userid."')";
			//$mysql->query($sql);
			...
			foreach($list as $item){
				...
				$field_add = '';
				$value_add = '';
				if(!empty($item['masteryscore']) && $item['masteryscore'] >= 0){
				...
				$title = $mysql->escape_string($item['title']);
				$max_score = $mysql->escape_string($item['max_score']);
				...
				/*if (!empty($_POST['index_document'])) {
					...
					isset($_POST['language'])? $lang=$mysql->escape_string($_POST['language']): $lang = 'english';
	 ...
	 */
	 ...
	 function set_parentDir($maker=''){
		 global $mysql;
		if($this->debug>0){error_log('In scorm::set_parentDir method('.$maker.')',0);}
	 	$lp = $this->get_id();
	 	if($lp!=0){
	 		$tbl_lp = "lp";
	...
	/**
	  ...
	  */
	  ...
	  function reimport_manifest($course,$lp_id=null,$imsmanifest_path=''){
		...
		//$course = $mysql->escape_string($course);
