...
<div id="content" class="maxcontent">
	...
	<div id="content_with_menu">
		<div id="container">
		<div class="info">课程单元（组）分配到课程单元组</div>
		<form action="fenpei.php?type=1" method="post">
			<div class="fenpei_dk fl">
				...
				<div style="max-height:210px;overflow-y: auto; overflow-x:hidden;">
					...
					<?php
						...
						while($arr = $mysql->fetch_array($res)){
							echo "<li title='创建日期：$arr[time]'><input type='checkbox' name='courseUnitGroup[]' value='$arr[id]'><img src='../img/no-expanded.gif' id=$arr[id] type=4> ".$arr["title"]."</li>";							
						}
					?>
					</ul>
					</div>
				...
				<div style="max-height:210px;overflow-y: auto; overflow-x:hidden;">
					...
					<?php
						...
						while($arr = $mysql->fetch_array($res)){
							echo "<li title='创建日期：$arr[time]'><label><input type='checkbox' name='courseUnit[]' value='$arr[id]'>".$arr["title"]."</label></li>";
						}
					?>
					</ul>
					</div>
			...
			<div class="fenpei_dk fr">
				...
				<div style="max-height:440px;overflow-y: auto; overflow-x:hidden;">
					...
					<?php
						$res = $mysql->query("select * from `group` where deleted=0 and type=2 $whereRole order by time desc");
						while($arr = $mysql->fetch_array($res)){
							echo "<li title='创建日期：$arr[time]'><label><input type='checkbox' name='toCourseUnitGroup[]' value='$arr[id]'>".$arr["title"]."</label></li>";
						}
					?>
					</ul>
					</div>
