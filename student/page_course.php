...
//考试状态定义
define("EXAM_STATUS_All",4);
define("EXAM_STATUS_UNDO",5);
define("EXAM_STATUS_EXPIRE",6);
...
//$select 是否处于分页复选模式
//$deleted 是否只显示已删除的用户
	function fenye ($page,$order,$linkPage,$userid,$keyword,$searchType=1,$startTime="",$endTime="",$status=0,$typeflag){
		...
		switch($status){
			...
			case STATUS_EXPIRE:
				$where .= " and (DATE_FORMAT( endTime, '%Y-%m-%d %H:%i:%S') <='".$timeNow."')";
				break;
            case EXAM_STATUS_All:
                break;
            case EXAM_STATUS_UNDO:
                $where .= " and (DATE_FORMAT( startTime, '%Y-%m-%d %H:%i:%S') >='".$timeNow."')";
                break;
            case EXAM_STATUS_EXPIRE:
                $where .= " and (DATE_FORMAT( endTime, '%Y-%m-%d %H:%i:%S') <='".$timeNow."')";
                break;
			}
